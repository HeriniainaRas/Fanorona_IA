...
def test_performance(depth=4, num_tests=10):
    ...
    print(f"Temps min/max: {min(minimax_times):.4f}s / {max(minimax_times):.4f}s")
    print(f"Nœuds explorés en moyenne: {total_nodes_minimax/num_tests:.0f}")
    print(f"Nœuds min/max: {min(minimax_nodes)} / {max(minimax_nodes)}")
    
    print("\nAlpha-Beta:")
    print(f"Temps moyen: {total_time_alphabeta/num_tests:.4f} secondes")
    print(f"Temps min/max: {min(alphabeta_times):.4f}s / {max(alphabeta_times):.4f}s")
    print(f"Nœuds explorés en moyenne: {total_nodes_alphabeta/num_tests:.0f}")
    print(f"Nœuds min/max: {min(alphabeta_nodes)} / {max(alphabeta_nodes)}")
